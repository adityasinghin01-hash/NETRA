...
export default function Briefing() {
  ...
  // One generator, three uses: on-screen "Print", "Download PDF" (both = the colour letterhead through
  // the browser print dialog, which prints paper OR saves PDF), and a "Fax-ready" export — a plain,
  // high-contrast, single-column monochrome sheet that survives a black-and-white fax transmission.
  function printBrief(mode: "print" | "fax") {
    if (!brief) return;
    const s = brief.stats;
    // Escape every dynamic value before it enters the print-window HTML (the briefing body is
    // model-authored text; on real data it could otherwise inject markup into document.write).
    const body = e(lang === "en" ? brief.en : brief.kn);
    const dist = e(brief.district);
    const dt = e(brief.date);
    const alertType = e(s.alert);
    const fcType = s.forecast ? e(s.forecast.crimeType.toLowerCase()) : "";
    const fcWindow = s.forecast ? e(s.forecast.patrolWindow) : "";
    const fcRisk = s.forecast ? e(s.forecast.riskLevel) : "";
    ...
    if (mode === "fax") {
      ...
      html = `<!doctype html><html><head><meta charset="utf-8"><title>NETRA FAX — ${dist}</title>
        <style>
          *{box-sizing:border-box} body{font-family:'Times New Roman',Georgia,serif;color:#000;background:#fff;margin:0}
          ...
          .faxhead{border:2px solid #000;padding:10px 14px;margin-bottom:18px}
          .faxhead .t{font-weight:800;letter-spacing:.1em;font-size:16px}
          .faxrow{display:flex;justify-content:space-between;gap:14px;font-size:12px;margin-top:6px}
          .faxrow b{text-transform:uppercase;letter-spacing:.04em}
          h1{font-size:19px;margin:0 0 2px} .sub{font-size:12px;margin-bottom:14px}
          .m{border-top:1.5px solid #000;border-bottom:1.5px solid #000;padding:8px 0;margin:12px 0;font-size:13px}
          .m span{margin-right:24px}
          h2{font-size:12px;text-transform:uppercase;letter-spacing:.07em;border-bottom:1px solid #000;padding-bottom:3px;margin:18px 0 8px}
          p{font-size:13px;line-height:1.7;margin:0}
          .bar{font-size:12px;margin:4px 0;font-family:'Courier New',monospace;letter-spacing:-.02em}
          .cal{border:1.5px solid #000;padding:8px 12px;font-size:12px;margin:10px 0}
          .foot{font-size:10px;margin-top:26px;border-top:1px solid #000;padding-top:10px;line-height:1.5}
          @media print{.page{padding:16px 20px}}
        </style></head><body><div class="page">
        <div class="faxhead">
          <div class="t">FACSIMILE &mdash; KARNATAKA STATE POLICE</div>
          <div class="faxrow"><span><b>To:</b> ______________________</span><span><b>From:</b> NETRA &middot; SP ${dist}</span></div>
          <div class="faxrow"><span><b>Date:</b> ${dt}</span><span><b>Pages:</b> 1</span><span><b>Re:</b> Daily Intelligence Briefing</span></div>
        </div>
        <h1>Daily Intelligence Briefing</h1>
        <div class="sub">${dist} District &middot; ${dt} &middot; DECISION SUPPORT</div>
        <div class="m"><span><b>30-day FIRs:</b> ${s.fir30d}</span><span><b>vs prev month:</b> ${chg}</span><span><b>Detection:</b> ${s.detectionPct}%</span></div>
        <h2>Executive summary</h2><p>${body}</p>
        <h2>Leading crime types (30 days)</h2>${bars}
        ${s.alert ? `<div class="cal"><b>ACTIVE ALERT:</b> rise in ${alertType} &mdash; recommend focused deployment.</div>` : ""}
        ${s.forecast ? `<div class="cal"><b>7-DAY FORECAST:</b> ${fcRisk} risk (${fcType}); patrol window ${fcWindow}.</div>` : ""}
        <div class="foot">Auto-generated by NETRA &middot; Decision support &mdash; verify against ground reports before acting (human-in-the-loop) &middot; Prototype: synthetic data. Transmitted via facsimile &mdash; monochrome high-contrast format.</div>
        </div></body></html>`;
    } else {
      const crimes = s.topCrimes.map((c) =>
        `<div class="bar-row"><span class="bar-label">${e(c.type)}</span>
      ...
      const alertHtml = s.alert
        ? `<div class="callout callout-alert"><div class="callout-h">⚠ Active alert</div>A rise in <b>${alertType}</b> has been flagged by anomaly detection — recommend focused deployment.</div>`
        : "";
      ...
      html = `<!doctype html><html><head><meta charset="utf-8"><title>NETRA Briefing — ${dist}</title>
        <style>
          *{box-sizing:border-box} body{font-family:Inter,system-ui,-apple-system,sans-serif;color:#0f1e33;margin:0;background:#fff}
          ...
          <div class="rule"></div>
          <h1>Daily Intelligence Briefing</h1>
          <div class="sub">${dist} District · ${dt}</div>
          <div class="metrics">
            ...
            <div className="mb-4 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  title="Print or save as PDF (colour letterhead)"
                >
                  🖨 Print / PDF
                </button>
                <button
                  onClick={() => printBrief("fax")}
                  className="rounded-lg border border-[var(--color-border-strong)] px-3 py-1 text-xs text-[var(--color-text-dim)] hover:text-[var(--color-accent)]"
                  title="Fax-ready — plain high-contrast monochrome sheet"
                >
                  📠 Fax-ready
                </button>
              </div>
              ...
              <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)] p-3">
                ...
                {patrol ? (
                  ...
                  <div className="text-xs text-[var(--color-text-dim)]">
                    No acute hotspot today — steady-state patrol.{brief.stats.leadType ? <> Watch <span className="text-[var(--color-text)]">{brief.stats.leadType}</span>, the district&rsquo;s leading crime type.</> : null}
                  </div>
                )}
              </div>
              <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)] p-3">
