  ...
  function printBrief(mode: "print" | "fax") {
    ...
    if (mode === "fax") {
      const bars = s.topCrimes.map((c) =>
        `<div class="bar">${c.type} — ${"█".repeat(Math.max(1, Math.round(c.pct / 3)))} ${c.pct}%</div>`).join("");
      html = `<!doctype html><html><head><meta charset="utf-8"><title>NETRA FAX — ${brief.district}</title>
        ...
        <div class="faxhead">
          ...
          <div class="faxrow"><span><b>To:</b> ______________________</span><span><b>From:</b> NETRA &middot; SP ${brief.district}</span></div>
          <div class="faxrow"><span><b>Date:</b> ${brief.date}</span><span><b>Pages:</b> 1</span><span><b>Re:</b> Daily Intelligence Briefing</span></div>
        </div>
        <h1>Daily Intelligence Briefing</h1>
        <div class="sub">${brief.district} District &middot; ${brief.date} &middot; DECISION SUPPORT</div>
        ...
        ${s.alert ? `<div class="cal"><b>ACTIVE ALERT:</b> rise in ${s.alert} &mdash; recommend focused deployment.</div>` : ""}
        ${s.forecast ? `<div class="cal"><b>7-DAY FORECAST:</b> ${s.forecast.riskLevel} risk (${s.forecast.crimeType.toLowerCase()}); patrol window ${s.forecast.patrolWindow}.</div>` : ""}
