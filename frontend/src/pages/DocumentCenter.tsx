  ...
  function linkageReport() {
    ...
    openReport({
      ...
      sections: [
        ...
        { heading: "E · NETRA AI Analysis", html: `<p>Linkage by multi-dimensional MO signature (semantic text + geography + time + method + target). ${sp?.rossmo ? `Geographic profiling (Rossmo/CGT) predicts an operating-base <b>zone</b> near ${sp.rossmo.anchor.lat}, ${sp.rossmo.anchor.lng} — a place, not a person.` : ""} ${sp?.nextStrike ? `Predicted next strike: <b>${sp.nextStrike.district}</b>, ${sp.nextStrike.window}, ${sp.nextStrike.timeWindow} (${Math.round(sp.nextStrike.confidence * 100)}% confidence).` : ""}</p>` },
