import React, { useRef, useEffect, ReactNode, MouseEventHandler, CSSProperties, ButtonHTMLAttributes } from 'react';
...
export function useSpecularCanvas(
  ...
  fxRef: React.RefObject<HTMLElement | null>,
  props: SpecularProps
) {
  ...
  useEffect(() => {
    ...
    const update = (now: number) => {
      ...
      const steer = (p.followMouse ?? true) && pointerAngle != null && isHovered;
      // CSS conic-gradient starts from top (12 o'clock), whereas atan2 is from right (3 o'clock).
      const targetAngle = steer ? pointerAngle + Math.PI / 2 : idleAngle;
